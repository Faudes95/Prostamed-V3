...
def test_field_router_bcr_salvage_window_minimum_is_compact_and_excludes_crpc_domains():
    from prostanet.presentation.clinical_field_router import build_clinical_field_router

    router = build_clinical_field_router(
        "recurrence_bcr",
    ...
    assert router["summary"]["readiness_filter_active"] is True
    assert {
        "psa_value",
    ...
    assert not any(token in blob for token in ("hrr", "parp", "castrate", "testosterone", "arpi", "ctcae", "lu177"))
    assert router["summary"]["total_fields"] <= 20
...
def test_wizard_form_uses_router_fields_when_readiness_lane_is_active():
    from prostanet.presentation.clinical_field_router import build_clinical_field_router
    from prostanet.presentation.views import _router_fields_for_wizard_form
    ...
    fields = _router_fields_for_wizard_form(router)
    names = {field["name"] for field in fields}

    assert len(fields) == router["summary"]["total_fields"]
    ...
    assert all(field.get("display_options") for field in fields if field.get("field_type") == "select")
