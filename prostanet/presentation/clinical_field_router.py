...
STATE_FIELD_ALLOWLISTS = {
    "recurrence_bcr": {
        ...
        "bcr_confirmed",
        "phoenix_criteria_met", "phoenix_failure_confirmed",
        ...
        "psa_rise_above_nadir_ng_ml", "phoenix_delta", "months_post_rt",
...
def _candidate_groups(
    ...
    seeds = _seed_groups(state, phase, readiness_lane=readiness_lane)
    return {
        "required": [
            *seeds.get("required", []),
...
def _name_allowlist_for_state(state: str) -> set[str] | None:
    ...
    if state in {"screening", "diagnostic_workup", "post_negative_biopsy_followup"}:
        return DIAGNOSTIC_FIELD_ALLOWLIST
...
def _seed_groups(state: str, phase: str, readiness_lane: str = "") -> dict[str, list[dict[str, Any]]]:
    ...
    if phase == "longitudinal_followup":
        ...
        if (
            readiness_lane in {
                ...
                "patient_twin_preference_pro_readiness",
                "patient_twin_preference_pro_minimum",
                "preference_function_baseline",
                "localized_function_preference_minimum",
                "localized_treatment_readiness",
            }
            or state in {"localized_initial", "recurrence_bcr", "m0_crpc", "m1_crpc", "palliative"}
        ):
            groups["decision_refiners"].extend(_fields(
                "patient_values",
                "baseline_pro",
...
SYNTHETIC_FIELDS: dict[str, dict[str, Any]] = {
    ...
    "psadt_months": {"label": "PSADT", "field_type": "number", "unit": "meses"},
    ...
    "bcr_confirmed": {"label": "BCR confirmada", "field_type": "select", "options": ["0", "1"]},
    ...
    "psa_rise_above_nadir_ng_ml": {"label": "Ascenso de PSA sobre nadir", "field_type": "number", "unit": "ng/mL"},
    "phoenix_delta": {"label": "Delta Phoenix", "field_type": "number", "unit": "ng/mL"},
    "phoenix_criteria_met": {"label": "Criterio Phoenix cumplido", "field_type": "select", "options": ["0", "1"]},
    "phoenix_failure_confirmed": {"label": "Falla Phoenix confirmada", "field_type": "select", "options": ["0", "1"]},
    "psma_pet_staging_recent": {"label": "PSMA PET reciente para rescate", "field_type": "select", "options": ["0", "1", "unknown"]},
    "psma_pet_status": {"label": "Estado PSMA PET", "field_type": "select", "options": ["not_done", "negative", "positive", "unknown"]},
    ...
    "patient_values": {"label": "Valores y prioridades del paciente", "field_type": "textarea"},
    "localized_patient_values": {"label": "Valores para decidir AS/RP/RT", "field_type": "textarea"},
    "baseline_pro": {"label": "PRO basal minimo documentado", "field_type": "textarea"},
    "toxicity_tolerance": {"label": "Tolerancia/toxicidad aceptable", "field_type": "textarea"},
    "decision_tradeoff": {"label": "Tradeoff de decision", "field_type": "textarea"},
    "redecision_threshold": {"label": "Umbral de nueva decision", "field_type": "textarea"},
    "urinary_function_baseline": {"label": "Funcion urinaria basal", "field_type": "number", "unit": "IPSS o escala local"},
    "sexual_function_baseline": {"label": "Funcion sexual basal", "field_type": "number", "unit": "IIEF-5 o escala local"},
    "bowel_function_baseline": {"label": "Funcion intestinal basal", "field_type": "textarea"},
    ...
    "dre_finding": {"label": "Hallazgo DRE", "field_type": "select", "options": ["normal", "suspicious", "unknown"]},
    "prior_negative_biopsy": {"label": "Biopsia previa negativa", "field_type": "select", "options": ["0", "1"]},
    "prior_negative_biopsy_documented": {"label": "Biopsia negativa documentada", "field_type": "select", "options": ["0", "1"]},
    "pirads_score": {"label": "PI-RADS", "field_type": "select", "options": ["0", "2", "3", "4", "5"]},
    "mri_pirads_score": {"label": "PI-RADS de mpMRI", "field_type": "select", "options": ["0", "2", "3", "4", "5"]},
    "prior_mpmri_pirads_score": {"label": "PI-RADS previo", "field_type": "select", "options": ["0", "2", "3", "4", "5"]},
    "psad": {"label": "Densidad PSA", "field_type": "number"},
    "psa_density": {"label": "Densidad PSA", "field_type": "number", "unit": "ng/mL/cc"},
    "prostate_volume_ml": {"label": "Volumen prostático", "field_type": "number", "unit": "mL"},
    "biopsy_status": {"label": "Estado de biopsia", "field_type": "select", "options": ["not_done", "planned", "negative", "positive", "unknown"]},
    "primary_biopsy_completed_or_planned": {"label": "Biopsia primaria completada o planeada", "field_type": "select", "options": ["0", "1"]},
    "primary_biopsy_not_performed": {"label": "Biopsia primaria no realizada", "field_type": "select", "options": ["0", "1"]},
    "targeted_biopsy_status": {"label": "Biopsia dirigida", "field_type": "select", "options": ["not_done", "planned", "completed", "unknown"]},
    "family_history": {"label": "Antecedente familiar relevante", "field_type": "select", "options": ["negative", "positive", "unknown"]},
    "family_history_positive": {"label": "Antecedente familiar positivo", "field_type": "select", "options": ["0", "1", "unknown"]},
    "germline_risk": {"label": "Riesgo germinal documentado", "field_type": "select", "options": ["low", "elevated", "known_mutation", "unknown"]},
    "germline_risk_mutation": {"label": "Mutación germinal conocida", "field_type": "select", "options": ["negative", "positive", "not_tested", "unknown"]},
    "germline_test_indicated": {"label": "Prueba germinal indicada", "field_type": "select", "options": ["0", "1", "unknown"]},
    "germline_family_history": {"label": "Historia familiar de riesgo germinal", "field_type": "select", "options": ["negative", "positive", "unknown"]},
    "phi_value": {"label": "PHI", "field_type": "number"},
    "fourkscore_value": {"label": "4Kscore", "field_type": "number"},
    "planned_biopsy_type": {"label": "Tipo de biopsia prevista", "field_type": "select", "options": ["systematic", "targeted", "combined", "not_planned", "unknown"]},
    "planned_biopsy_route": {"label": "Via de biopsia prevista", "field_type": "select", "options": ["transperineal", "transrectal", "not_planned", "unknown"]},
