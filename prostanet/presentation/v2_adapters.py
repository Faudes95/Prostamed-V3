...
def quick_classify_schema() -> dict[str, Any]:
    ...
    schema = {
        ...
        "title": "Intake clínico completo NCCN 5.2026 + EAU 2026 · cobertura total · progressive disclosure",
        "description": "Cobertura clínica COMPLETA (NO eliminamos campos por reducir captura — usamos progressive disclosure + voice para manejar abundancia). Cubre los 53 estados clínicos + discriminadores risk-tier + imaging detalle + germline panel + fitness geriátrico + comorbilidades drug-selection + PROs baseline + preferencias paciente + DDI. Los campos se muestran progresivamente según respuestas previas. EPIC 42.B expansión: +30 fields agrupados en 8 nuevos clusters (Biopsia detalle / Imaging detalle / Genética molecular / Fitness geriátrico / Comorbilidades específicas / PROs baseline / Preferencias paciente / Medicación).",
        "fields": [
            ...
            # ──────────────────────────────────────────────────────────────
            # EPIC 42.B.2 — COBERTURA CLÍNICA COMPLETA (NO eliminar campos)
            # 8 nuevos grupos progresivos para captura de discriminadores
            # fundamentales NCCN/EAU 2026 que estaban ausentes del intake.
            # ──────────────────────────────────────────────────────────────

            # ── BIOPSIA DETALLE (granular pathology) — known_cancer_diagnosis=1 ──
            {"name": "total_cores_biopsied", "label": "Total de cores biopsiados (N)", "field_type": "number", "required": False, "min_value": 1, "max_value": 50, "group": "Biopsia detalle", "group_order": 5.5, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "NCCN PROS-2: required para very_low_risk classification (umbral <34% positive cores)"},
            {"name": "cores_positive_count", "label": "Cores positivos (N)", "field_type": "number", "required": False, "min_value": 0, "max_value": 50, "group": "Biopsia detalle", "group_order": 5.5, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "% positive cores = cores_positive / total_cores. Discrimina very_low/low/favorable_intermediate"},
            {"name": "percent_positive_cores", "label": "% cores positivos (si conocido)", "field_type": "number", "required": False, "min_value": 0, "max_value": 100, "unit": "%", "group": "Biopsia detalle", "group_order": 5.5, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "Auto-derivado de total + positives, o ingresar manualmente"},
            {"name": "percent_pattern_4", "label": "% patrón Gleason 4 en core más afectado", "field_type": "number", "required": False, "min_value": 0, "max_value": 100, "unit": "%", "group": "Biopsia detalle", "group_order": 5.5, "conditional_visibility": {"gleason_secondary": ["4", "5"]}, "help_text": "Discrimina favorable_intermediate (<50%) vs unfavorable_intermediate (≥50%) en GS 3+4=7"},
            {"name": "perineural_invasion", "label": "Invasión perineural (PNI)", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Biopsia detalle", "group_order": 5.5, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "PNI+ = factor pronóstico adverso (Maru J Urol 2001), refuerza decisión adjuvant"},
            {"name": "biopsy_route", "label": "Vía de biopsia", "field_type": "select", "required": False, "options": ["unknown", "transrectal_systematic", "transperineal_systematic", "mri_fusion_targeted", "mri_fusion_plus_systematic", "cognitive_targeted"], "default": "unknown", "group": "Biopsia detalle", "group_order": 5.5, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "MRI-fusion+systematic = gold standard NCCN PROS-1 v2026"},
            {"name": "extracapsular_extension_on_biopsy", "label": "Extensión extracapsular en biopsia", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Biopsia detalle", "group_order": 5.5, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "EPE en biopsia eleva a cT3a, modifica candidacy AS"},
            {"name": "prostate_volume_ml", "label": "Volumen prostático (mL, ecografía/MRI)", "field_type": "number", "required": False, "min_value": 5, "max_value": 500, "unit": "mL", "group": "Biopsia detalle", "group_order": 5.5, "help_text": "Requerido para calcular PSA density (PSA/volumen)"},
            {"name": "psa_density", "label": "PSA densidad (auto-derivada o manual)", "field_type": "number", "required": False, "min_value": 0, "max_value": 5, "unit": "ng/mL/cc", "group": "Biopsia detalle", "group_order": 5.5, "help_text": "PSAD <0.15 = criterio very_low_risk_localized. Auto-calculada si PSA + volumen presentes"},

            # ── IMAGING DETALLE — known_cancer_diagnosis=1 ──
            {"name": "mpmri_done", "label": "mpMRI prostática realizada", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "NCCN PROS-1 v2026: mpMRI antes de biopsia inicial (favorable_intermediate+)"},
            {"name": "mpmri_date", "label": "Fecha mpMRI", "field_type": "date", "required": False, "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"mpmri_done": ["1"]}},
            {"name": "mpmri_lesion_count", "label": "Lesiones PI-RADS≥3 en mpMRI", "field_type": "number", "required": False, "min_value": 0, "max_value": 20, "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"mpmri_done": ["1"]}},
            {"name": "bone_scan_done", "label": "Gammagrafía ósea realizada", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "Indicada en NCCN unfavorable_intermediate+ o GS ≥8 o PSA ≥20"},
            {"name": "bone_scan_date", "label": "Fecha gammagrafía ósea", "field_type": "date", "required": False, "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"bone_scan_done": ["1"]}},
            {"name": "ct_abdomen_pelvis_done", "label": "CT abdomen+pelvis realizada", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}},
            {"name": "psma_pet_done", "label": "PSMA-PET realizada", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "NCCN PROS-D v2026 preferred over conventional en BCR y staging mCRPC"},
            {"name": "psma_pet_date", "label": "Fecha PSMA-PET", "field_type": "date", "required": False, "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"psma_pet_done": ["1"]}, "help_text": "psma_study_date — usado por Trial Matcher VISION/PSMAfore"},
            {"name": "psma_tracer", "label": "Trazador PSMA usado", "field_type": "select", "required": False, "options": ["unknown", "Ga-68_PSMA-11", "F-18_PYL", "F-18_rhPSMA-7.3", "Ga-68_PSMA-617", "other"], "default": "unknown", "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"psma_pet_done": ["1"]}},
            {"name": "psma_positive", "label": "PSMA-PET positivo (avid disease)", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"psma_pet_done": ["1"]}, "help_text": "Required para VISION/PSMAfore (Lu-177 PSMA-617) elegibilidad"},
            {"name": "psma_lesion_count", "label": "Lesiones PSMA-avid (N)", "field_type": "number", "required": False, "min_value": 0, "max_value": 500, "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"psma_positive": ["1"]}, "help_text": "≤5 = candidato oligometastatic SBRT (STOMP/ORIOLE)"},
            {"name": "psma_index_lesion_suvmax", "label": "SUVmax lesión índice", "field_type": "number", "required": False, "min_value": 0, "max_value": 200, "group": "Imaging detalle", "group_order": 5.7, "conditional_visibility": {"psma_positive": ["1"]}, "help_text": "SUVmax ≥ hígado de referencia = PSMA-positive criterio VISION"},

            # ── MOLECULAR / GERMLINE — NCCN PROS-A v2026 universal ──
            {"name": "germline_testing_performed", "label": "Germline testing realizado", "field_type": "select", "required": False, "options": ["unknown", "0", "1", "pending"], "default": "unknown", "group": "Genética molecular", "group_order": 6.5, "help_text": "NCCN PROS-A v2026: testing universal en PCa avanzado/metastásico/familia high-risk"},
            {"name": "germline_testing_date", "label": "Fecha resultado germline", "field_type": "date", "required": False, "group": "Genética molecular", "group_order": 6.5, "conditional_visibility": {"germline_testing_performed": ["1"]}},
            {"name": "hrr_status", "label": "Status HRR/BRCA", "field_type": "select", "required": False, "options": ["unknown", "negative", "positive", "vus_only"], "default": "unknown", "group": "Genética molecular", "group_order": 6.5, "help_text": "Positive en BRCA1/2, ATM, PALB2, CDK12, CHEK2, FANCA, RAD51B/C/D, BARD1 — PARP-eligible"},
            {"name": "hrr_gene", "label": "Gen patogénico (si HRR+)", "field_type": "select", "required": False, "options": ["", "BRCA2", "BRCA1", "ATM", "PALB2", "CDK12", "CHEK2", "FANCA", "RAD51B", "RAD51C", "RAD51D", "BARD1", "HOXB13", "OTHER"], "default": "", "group": "Genética molecular", "group_order": 6.5, "conditional_visibility": {"hrr_status": ["positive"]}, "help_text": "BRCA2 = highest PARP response (PROfound). HOXB13 G84E = surveillance intensified"},
            {"name": "biomarker_source", "label": "Fuente del biomarcador", "field_type": "select", "required": False, "options": ["unknown", "germline", "somatic_tumor", "ctdna", "both_germline_somatic"], "default": "unknown", "group": "Genética molecular", "group_order": 6.5, "conditional_visibility": {"hrr_status": ["positive"]}, "help_text": "Germline = hereditario (counseling familiar). Somatic = solo tumor (cribar germline también)"},
            {"name": "msi_status", "label": "MSI / dMMR status", "field_type": "select", "required": False, "options": ["unknown", "stable", "high", "indeterminate"], "default": "unknown", "group": "Genética molecular", "group_order": 6.5, "help_text": "MSI-H/dMMR = pembrolizumab eligible (KEYNOTE-199)"},
            {"name": "first_degree_relative_pca_lt60", "label": "Familiar 1er grado con CA próstata <60 años", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Genética molecular", "group_order": 6.5, "help_text": "Trigger NCCN PROS-A para germline testing"},
            {"name": "first_degree_relative_brca_breast_ovarian", "label": "Familiar 1er grado con BRCA / mama / ovario / páncreas", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Genética molecular", "group_order": 6.5, "help_text": "Trigger NCCN PROS-A para germline + counseling familiar"},
            {"name": "lynch_syndrome_features", "label": "Lynch syndrome features (colon/endometrio/MLH1/MSH2)", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Genética molecular", "group_order": 6.5, "help_text": "Lynch+ → pembrolizumab eligibility + screening colon/endometrio"},
            {"name": "genomic_classifier_result", "label": "Genomic classifier (Decipher / Prolaris)", "field_type": "select", "required": False, "options": ["", "decipher_low", "decipher_intermediate", "decipher_high", "prolaris_low", "prolaris_intermediate", "prolaris_high", "oncotype_low", "oncotype_high", "not_performed"], "default": "not_performed", "group": "Genética molecular", "group_order": 6.5, "conditional_visibility": {"known_cancer_diagnosis": ["1"]}, "help_text": "Decipher >0.6 = high genomic risk, refuerza intensified treatment en intermediate-risk localized"},
            {"name": "genomic_classifier_report_date", "label": "Fecha resultado genomic classifier", "field_type": "date", "required": False, "group": "Genética molecular", "group_order": 6.5, "conditional_visibility": {"genomic_classifier_result": ["decipher_low", "decipher_intermediate", "decipher_high", "prolaris_low", "prolaris_intermediate", "prolaris_high"]}},

            # ── FITNESS / GERIATRIC — NCCN Older Adult Oncology 2026 ──
            {"name": "g8_score", "label": "G8 score (geriatric screening, >70y)", "field_type": "number", "required": False, "min_value": 0, "max_value": 17, "group": "Fitness geriátrico", "group_order": 6.7, "help_text": "G8 ≤14 = fragility; treatment de-escalation. >70y obligatorio NCCN Older Adult v2026"},
            {"name": "frailty_status", "label": "Frailty status (Fried/CGA)", "field_type": "select", "required": False, "options": ["unknown", "robust", "pre_frail", "frail", "severe_frail"], "default": "unknown", "group": "Fitness geriátrico", "group_order": 6.7, "help_text": "Frail/severe_frail → de-escalate cirugía, RT corta, ARPI con dose reduction"},
            {"name": "mini_cog_score", "label": "Mini-Cog (cognitive screen, >70y)", "field_type": "number", "required": False, "min_value": 0, "max_value": 5, "group": "Fitness geriátrico", "group_order": 6.7, "help_text": "≤3 = cognitive impairment, evitar enzalutamide (riesgo cognitivo)"},
            {"name": "life_expectancy_years_estimated", "label": "Esperanza de vida estimada (años)", "field_type": "number", "required": False, "min_value": 0, "max_value": 50, "unit": "años", "group": "Fitness geriátrico", "group_order": 6.7, "help_text": "<10 años = AS preferida en localized. >15 años = treatment más agresivo justificado"},
            {"name": "anesthesia_surgical_fitness", "label": "Aptitud quirúrgica/anestésica (RP candidato)", "field_type": "select", "required": False, "options": ["unknown", "fit", "borderline", "unfit"], "default": "unknown", "group": "Fitness geriátrico", "group_order": 6.7, "help_text": "Determina elegibilidad RP vs RT-only"},
            {"name": "radiotherapy_feasibility", "label": "Aptitud radioterapéutica", "field_type": "select", "required": False, "options": ["unknown", "feasible", "limited_pelvic_rt_prior", "contraindicated"], "default": "unknown", "group": "Fitness geriátrico", "group_order": 6.7, "help_text": "RT previa pélvica = contraindica re-irradiation"},

            # ── COMORBILIDADES DETALLADAS — drug-selection safety ──
            {"name": "severe_cv_disease", "label": "Enfermedad cardiovascular severa", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Comorbilidades específicas", "group_order": 7.5, "help_text": "1 = IC NYHA III-IV, MI <6m, arritmia maligna → evitar abiraterone (mineralocorticoide)"},
            {"name": "active_liver_disease", "label": "Disfunción hepática activa (LFT >3x ULN)", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Comorbilidades específicas", "group_order": 7.5, "help_text": "1 → abiraterone contraindicado, preferir enzalutamide/apalutamide"},
            {"name": "cognitive_impairment_documented", "label": "Deterioro cognitivo documentado", "field_type": "select", "required": False, "options": ["unknown", "0", "1"], "default": "unknown", "group": "Comorbilidades específicas", "group_order": 7.5, "help_text": "1 → evitar enzalutamide (fatiga + caídas + cognitive AEs)"},
            {"name": "diabetes_baseline_a1c", "label": "HbA1c basal", "field_type": "number", "required": False, "min_value": 4, "max_value": 18, "unit": "%", "group": "Comorbilidades específicas", "group_order": 7.5, "help_text": "ADT empeora metabolismo; baseline para monitoreo"},
            {"name": "dxa_t_score_lumbar", "label": "DEXA T-score columna lumbar (baseline)", "field_type": "number", "required": False, "min_value": -5, "max_value": 5, "group": "Comorbilidades específicas", "group_order": 7.5, "help_text": "T<-2.5 = osteoporosis → bisfosfonato/denosumab antes ADT prolongado"},
            {"name": "dxa_t_score_hip", "label": "DEXA T-score cadera", "field_type": "number", "required": False, "min_value": -5, "max_value": 5, "group": "Comorbilidades específicas", "group_order": 7.5},
            {"name": "renal_creatinine_clearance", "label": "Aclaramiento de creatinina (mL/min)", "field_type": "number", "required": False, "min_value": 0, "max_value": 200, "unit": "mL/min", "group": "Comorbilidades específicas", "group_order": 7.5, "help_text": "<30 = contraindica rucaparib + ajuste cabazitaxel/Lu-177"},

            # ── PROs BASELINE — quality of life ──
            {"name": "ipss_total", "label": "IPSS (síntomas urinarios)", "field_type": "number", "required": False, "min_value": 0, "max_value": 35, "group": "PROs baseline", "group_order": 7.7, "help_text": "0-7 leve · 8-19 moderado · 20-35 severo. IPSS alto = contraindicación relativa brachy"},
            {"name": "iief5_score", "label": "IIEF-5 (función eréctil baseline)", "field_type": "number", "required": False, "min_value": 0, "max_value": 25, "group": "PROs baseline", "group_order": 7.7, "help_text": "Baseline para comparar pre/post-tratamiento. <17 = disfunción significativa"},
            {"name": "epic26_urinary_domain", "label": "EPIC-26 urinary domain", "field_type": "number", "required": False, "min_value": 0, "max_value": 100, "group": "PROs baseline", "group_order": 7.7, "help_text": "Higher=better. Baseline urinary QoL"},
            {"name": "epic26_sexual_domain", "label": "EPIC-26 sexual domain", "field_type": "number", "required": False, "min_value": 0, "max_value": 100, "group": "PROs baseline", "group_order": 7.7},
            {"name": "epic26_bowel_domain", "label": "EPIC-26 bowel domain", "field_type": "number", "required": False, "min_value": 0, "max_value": 100, "group": "PROs baseline", "group_order": 7.7, "help_text": "Baseline para evaluar toxicidad rectal post-RT"},
            {"name": "epic26_hormonal_domain", "label": "EPIC-26 hormonal domain", "field_type": "number", "required": False, "min_value": 0, "max_value": 100, "group": "PROs baseline", "group_order": 7.7, "help_text": "Baseline pre-ADT para medir hot flashes, fatiga, libido decline"},

            # ── PREFERENCIAS PACIENTE / SDM ──
            {"name": "goal_of_care", "label": "Goal of care (objetivo dominante)", "field_type": "select", "required": False, "options": ["unknown", "maximize_survival", "preserve_qol", "balanced", "palliative_focus"], "default": "unknown", "group": "Preferencias paciente", "group_order": 8.5, "help_text": "Drive SDM. preserve_qol → AS preferida en low-risk; palliative → de-escalate"},
            {"name": "patient_priority_profile", "label": "Prioridad de preservación (vs efectos)", "field_type": "select", "required": False, "options": ["unknown", "urinary_continence_first", "sexual_function_first", "bowel_function_first", "minimal_visits_first", "no_priority"], "default": "unknown", "group": "Preferencias paciente", "group_order": 8.5, "help_text": "Sexual=evitar RP nerve-non-sparing. Urinary=evitar brachy en IPSS alto. Bowel=evitar EBRT"},
            {"name": "treatment_modality_preference", "label": "Preferencia inicial de tratamiento", "field_type": "select", "required": False, "options": ["none", "surveillance", "surgery", "radiation", "brachytherapy", "focal", "systemic_only"], "default": "none", "group": "Preferencias paciente", "group_order": 8.5, "help_text": "Capturar preferencia ANTES de recomendación; comparar contra NCCN para detectar misalignment"},

            # ── MEDICACIÓN ACTUAL / DDI ──
            {"name": "current_medications", "label": "Medicación actual relevante (lista libre)", "field_type": "textarea", "required": False, "group": "Medicación", "group_order": 9.5, "help_text": "Capturar warfarina/DOAC, beta-bloqueador, statin, antiepiléptico — DDI screen importante con ARPI"},
            {"name": "ddi_review_status", "label": "DDI review status", "field_type": "select", "required": False, "options": ["pending", "completed_no_interactions", "completed_interactions_managed", "completed_blocking_interaction"], "default": "pending", "group": "Medicación", "group_order": 9.5, "help_text": "blocking_interaction → terapia alternativa requerida (e.g., enzalutamide CYP3A4 inducer con warfarina)"},

            # ── TESTOSTERONE SAMPLE DATE — EPIC 31.E staleness sentinel ──
            {"name": "testosterone_sample_date", "label": "Fecha de medición testosterona", "field_type": "date", "required": False, "group": "CRPC verification", "group_order": 10, "conditional_visibility": {"current_adt_context": ["medical_adt_continuous", "orchiectomy", "intermittent_adt"]}, "help_text": "Si >90d → castration_status no confiable, repetir antes de clasificar CRPC"},
        ],
    }
