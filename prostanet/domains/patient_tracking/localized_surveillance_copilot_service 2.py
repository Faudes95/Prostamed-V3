...
def _concordance_label(rule_action: str, overlay_action: str) -> str:
    ...
    if rule_text == overlay_text:
        return "concordant"
    # Guard L-2 (FAUBOT): los tokens bilingües cubren EN + ES para que la
    # detección de "adjacent" no falle en recomendaciones escritas en español.
    surveillance_tokens = ("surveillance", "vigilanc")
    radiation_tokens = ("radi", "radioterap", "rt ", "radioter")
    prostatectomy_tokens = ("prostatect",)
    if any(tok in rule_text for tok in surveillance_tokens) and any(tok in overlay_text for tok in surveillance_tokens):
        return "adjacent"
    if any(tok in rule_text for tok in radiation_tokens) and any(tok in overlay_text for tok in radiation_tokens):
        return "adjacent"
    if any(tok in rule_text for tok in prostatectomy_tokens) and any(tok in overlay_text for tok in prostatectomy_tokens):
        return "adjacent"
    return "discordant"
...
class LocalizedSurveillanceCopilotService:
    ...
    def evaluate(
        ...
        *,
        ...
        if management_track_text == "active_surveillance" or any(
            ...
            # Guard L-1 (FAUBOT): evitar fabricar fechas. Si no hay diagnosis_date ni
            # first_positive_biopsy_date reales, dejamos enrollment_date vacío para
            # que el perfil oficial muestre "sin dato" y el motor pida el campo,
            # en lugar de sostener una fecha sintética ("2026-01-01") que falseaba
            # el tiempo en vigilancia activa.
            enrollment_source = normalize_text(
                payload.get("diagnosis_date") or payload.get("first_positive_biopsy_date") or ""
            )
            as_context.setdefault("enrollment_date", enrollment_source)
            if not enrollment_source:
                as_context.setdefault("enrollment_date_missing", True)
        ...
        # EPIC 5 — engine serial longitudinal. Enriquece `active_triggers`
        # con señales que sólo aparecen al cruzar ≥3 snapshots (aceleración
        # PSA, PSADT log-lineal, escalada Decipher, ansiedad sostenida).
        # Dedupe por (trigger_type, severity) para no duplicar la señal
        # cross-seccional que ya produce `evaluate_reclassification`.
        longitudinal_report = LongitudinalASService.evaluate(as_patient)
        if longitudinal_report.snapshots_evaluated >= 2:
            existing_keys = {
                (t.get("trigger_type"), t.get("severity"))
                for t in (as_summary.get("active_triggers") or [])
            }
            merged_triggers = list(as_summary.get("active_triggers") or [])
            for trigger in longitudinal_report.triggers:
                key = (trigger.trigger_type, trigger.severity)
                if key in existing_keys:
                    continue
                trigger_dict = trigger.to_dict()
                trigger_dict["source"] = "longitudinal"
                merged_triggers.append(trigger_dict)
                existing_keys.add(key)
            as_summary["active_triggers"] = merged_triggers
            as_summary["trigger_count"] = len(merged_triggers)
            # Si el engine serial detecta probabilidad alta, elevar el tono
            # del card sin degradar una alerta cross-seccional ya existente.
        ...
        as_summary["longitudinal_report"] = longitudinal_report.to_dict()
        ...
        return {
            ...
            "as_longitudinal_report": longitudinal_report.to_dict(),
            ...
            "why_changed_today": self._why_changed_today(
                payload, localized_track, as_summary,
            ...
            # y not_recommended desde localized_initial (STAMPEDE arm G:
            # RT+ADT+abiraterona en VERY HIGH risk).
    ...
    def _why_changed_today(
        ...
        return notes[:7]  # +2 slots por las nuevas narrativas
