...
class SequencePlan:
    ...
    # EPIC 9 Group A — GAP-11: alertas estructuradas por biomarcador (evidence_tag,
    # alert_family) para que alert_engine y profile_compass puedan presentarlas
    # sin duplicar heurísticas. Para AR-V7+ referencia PROPHECY (Armstrong 2019).
    biomarker_alerts: list[dict[str, Any]] = field(default_factory=list)
...
SEQUENCE_LIBRARY: dict[str, list[dict[str, Any]]] = {
    ...
    # EPIC 3.7 — NEPC transformation (de novo o t-NEPC post-ARPI prolongado)
    # Activa cuando biomarkers.nepc = True (ver _library_keys_for_state).
...
class TreatmentSequencer:
    ...
    def optimize(self, patient: dict[str, Any]) -> SequencePlan:
        ...
        biomarker_alerts = self._build_biomarker_alerts(biomarkers)
    ...
    def _extract_biomarkers(
        ...
        follow_ups = patient.get("follow_ups", []) or []
        latest_fu = follow_ups[-1] if follow_ups else {}
        # EPIC 3.7 — leer bundles nuevos del último result snapshot si existe.
        latest_assessment = patient.get("latest_assessment") or {}
        latest_result = dict(latest_assessment.get("result_snapshot") or {})
        ...
        return {
            ...
            or bool(latest_result.get("nccn_primary", {}).get("dmmr_high_confidence")),
            "arv7_positive": bool(
                genomic.get("arv7_positive")
                or baseline.get("arv7_positive")
                or str(baseline.get("ar_v7_status", "")).lower().startswith("pos")
            ),
            "psma_positive": bool(
    ...
    def _build_sequence(
        ...
        # EPIC 3.7 — pasar biomarkers para que NEPC active librería platino.
    ...
    def _library_keys_for_state(
        ...
        EPIC 3.7: cuando biomarkers.nepc == True, la ruta NEPC platino
        tiene precedencia sobre librerías ARPI/taxano estándar en m1_crpc.
        """
        ...
        if state in ("mcspc_high_volume_sync", "mcspc_high_volume_metachronous", "mcspc_high_volume"):
            ...
            # NEPC transformation confirmada/sospechada → librería platino primero
            # (NEPC_TRANSFORMATION_PREFER); ARPI y docetaxel_adt quedan excluidos
            # de forma automática vía NEPC_TRANSFORMATION_AVOID.
            if biomarkers.get("nepc"):
                keys.append("m1_crpc_nepc_transformation")
                # Mantener línea post-ARSI como fallback (cabazitaxel, Lu-177,
                # radio-223) si ya hubo ARPI y el comité elige no platino.
    ...
    def _build_rationale(
        ...
        if biomarkers.get("arv7_positive"):
            # EPIC 9 Group A GAP-11 — evidence_tag PROPHECY expuesto en narrativa;
            # la alerta estructurada equivalente se publica en plan.biomarker_alerts.
            parts.append(
                "AR-V7 positivo (PROPHECY, Armstrong JAMA Oncol 2019, cat 1): se eliminan "
                "ARSI de la secuencia por baja eficacia. Se prioriza taxano."
            )
        if biomarkers.get("nepc"):
    ...
    @staticmethod
    def _build_biomarker_alerts(biomarkers: dict[str, Any]) -> list[dict[str, Any]]:
        """EPIC 9 Group A GAP-11 — Alertas estructuradas por biomarcador.

        Cada alerta incluye `alert_family` y `evidence_tag` para que
        alert_engine / profile_compass las presenten sin reimplementar la
        heurística. Hoy sólo AR-V7+ (PROPHECY, Armstrong 2019), ampliable a
        NEPC, MSI-H, HRR+ cuando se añadan reglas paralelas.
        """
            ...
            alerts.append(
                {
                    "biomarker": "AR-V7",
                    "status": "positive",
                    "alert_family": "biomarker_contraindication",
                    "severity": "contraindicated",
                    "evidence_tag": "PROPHECY_Armstrong_JAMA_Oncol_2019",
                    "evidence_level": "category_1",
                    "avoid_drugs": sorted(ARV7_POSITIVE_AVOID),
                    "preferred_alternative_family": "taxane",
                    "message": (
                        "AR-V7 positivo en CTC (PROPHECY, Armstrong JAMA Oncol 2019): la "
                        "respuesta a ARSI está significativamente atenuada; priorizar taxano."
                    ),
                }
            )
        return alerts
