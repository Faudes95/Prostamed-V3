...
def build_care_intent_contract(
    *,
    ...
    persistent_psa_pending_post_rp = (
        state == "post_prostatectomy"
        and derive_post_prostatectomy_course(patient) == "persistent_psa"
    )
    if persistent_psa_pending_post_rp:
    ...
    # Auditoría #21 (cierre OOS-3): un next_best_action con título de acción terapéutica
    # activa (Priorizar salvage/MDT, Redirigir a sistémica, Activar salvage, Iniciar
    # evaluación temprana) ya refleja una decisión oncológica operativa emitida por el
    # copiloto del dominio. Survivorship monitoring NO debe tapar esa narrativa:
    # la guía NCCN/EAU prioriza el rescate local viable sobre recuperación funcional
    # mientras la ventana curativa esté abierta. Ver `longitudinal_intelligence.py:1965-1972`
    # para la emisión original de "Priorizar {dominant_label}" en local_salvage_candidate.
    incoming_title = str(next_best_action.get("title") or "").strip()
    _active_therapy_prefixes = (
        "Priorizar ",
        "Sostener salvage",
        "Redirigir a secuencia sist\u00e9mica",
        "Activar salvage",
        "Iniciar evaluaci\u00f3n",
    )
    next_action_is_active_pathway = any(
        incoming_title.startswith(prefix) for prefix in _active_therapy_prefixes
    )
    survivorship_dominant = (
        survivorship_active
        and not palliative_active
        and not next_action_is_active_pathway
        and (
...
def _resolve_runtime_regimen_contracts(
    ...
    scope_contract = dict(
        ...
        or build_systemic_regimen_scope_contract(state, module_result)
    )
    return {
        ...
        "comparative_eligibility_matrix": dict(module_result.get("comparative_eligibility_matrix") or {}),
        "systemic_regimen_scope": str(module_result.get("systemic_regimen_scope") or scope_contract.get("scope") or "not_applicable"),
        "systemic_regimen_scope_contract": scope_contract,
    }
...
def _latest_positive_conventional_imaging(patient: dict[str, Any]) -> dict[str, Any]:
    imaging = sorted(patient.get("imaging") or [], key=lambda item: str(item.get("study_date") or ""), reverse=True)
    for study in imaging:
        study_type = str(study.get("study_type", "")).lower()
        findings = study.get("findings", {}) if isinstance(study.get("findings"), dict) else {}
        status = str(findings.get("conventional_imaging_status") or study.get("conventional_imaging_status") or "").upper()
        if status.startswith("M1") and any(token in study_type for token in ("tac", "gammagrama", "ct", "bone")):
            return study
        if "gammagrama" in study_type and str(study.get("bone_scan_result", "")).lower().startswith("positivo"):
            return study
        if "tac" in study_type and (
            str(findings.get("ct_summary") or "") == "Metástasis"
            or bool(findings.get("ct_locations"))
        ):
            return study
    return {}


def _latest_positive_psma_imaging(patient: dict[str, Any]) -> dict[str, Any]:
    imaging = sorted(patient.get("imaging") or [], key=lambda item: str(item.get("study_date") or ""), reverse=True)
    for study in imaging:
        study_type = str(study.get("study_type", "")).lower()
        findings = study.get("findings", {}) if isinstance(study.get("findings"), dict) else {}
        psma_stage = str(study.get("psma_stage_after_psma") or findings.get("psma_stage_after_psma") or "").upper()
        if "psma" in study_type and (
            psma_stage.startswith("M1")
            or _safe_int(findings.get("psma_total_lesions"))
            or str(study.get("psma_result") or "").lower() in {"positivo", "positive", "diseminado"}
        ):
            return study
    return {}


def _infer_metastatic_sensitive_target_state(patient: dict[str, Any], field_values: dict[str, Any]) -> str:
    burden_context = derive_mhspc_burden_context(field_values)
    metachronous = str(field_values.get("metachronous_metastasis") or "").strip().lower() in {"1", "true", "yes", "si", "sí"}
    if not metachronous:
        metachronous = bool(patient.get("surgery") or patient.get("radiation"))
    volume_disease = str(burden_context.get("volume_disease") or "low").lower()
    metastasis_count = _safe_int(field_values.get("metastasis_count") or burden_context.get("metastasis_count")) or 0
    if volume_disease == "high":
        return "mcspc_high_volume_metachronous" if metachronous else "mcspc_high_volume_sync"
    if burden_context.get("oligometastatic_operational") or metastasis_count <= 5:
        return "mcspc_oligo_metachronous" if metachronous else "mcspc_low_volume_sync_oligo"
    return "mcspc_high_volume_metachronous" if metachronous else "mcspc_high_volume_sync"


def _derive_metastatic_state_bundle(
    patient: dict[str, Any],
    ...
    psma_profile = dict(patient.get("psma_structured_profile") or {})
    latest_positive_psma = _latest_positive_psma_imaging(patient)
    latest_positive_conventional = _latest_positive_conventional_imaging(patient)

    metastatic_payload = dict(patient.get("baseline") or {})
    for source in (
        truth_values,
        latest_signal_snapshot,
        latest_inputs,
    ...
    if latest_positive_psma:
        psma_findings = latest_positive_psma.get("findings", {}) if isinstance(latest_positive_psma.get("findings"), dict) else {}
        metastatic_payload.setdefault("psma_positive", 1)
        metastatic_payload.setdefault(
            "psma_stage_after_psma",
            latest_positive_psma.get("psma_stage_after_psma") or psma_findings.get("psma_stage_after_psma"),
        )
        metastatic_payload.setdefault("psma_result", latest_positive_psma.get("psma_result"))
        metastatic_payload.setdefault("psma_study_date", latest_positive_psma.get("study_date"))
        metastatic_payload.setdefault("study_date", latest_positive_psma.get("study_date"))
        metastatic_payload.setdefault("metastasis_assessment_date", latest_positive_psma.get("study_date"))
        metastatic_payload.setdefault("metastasis_document_source", "psma_pet")
    if latest_positive_conventional:
        conventional_findings = latest_positive_conventional.get("findings", {}) if isinstance(latest_positive_conventional.get("findings"), dict) else {}
        metastatic_payload.setdefault(
            "conventional_imaging_status",
            conventional_findings.get("conventional_imaging_status") or latest_positive_conventional.get("conventional_imaging_status") or "M1",
        )
        metastatic_payload.setdefault("conventional_imaging_date", latest_positive_conventional.get("study_date"))
        metastatic_payload.setdefault("conventional_imaging_modality", latest_positive_conventional.get("study_type"))
        metastatic_payload.setdefault("metastasis_assessment_date", latest_positive_conventional.get("study_date"))
        metastatic_payload.setdefault("metastasis_document_source", "conventional_imaging")

    metastatic_payload.setdefault(
        "conventional_imaging_status",
        latest_inputs.get("conventional_imaging_status")
        or truth_values.get("conventional_imaging_status")
        or _derive_conventional_imaging_status(patient, state),
    )
    metastatic_payload.setdefault(
        "psma_stage_after_psma",
        psma_profile.get("psma_stage_after_psma"),
    )
    metastatic_payload.setdefault(
        "psma_positive",
        psma_profile.get("psma_positive"),
    )
    metastatic_payload.setdefault(
        "psma_study_date",
        psma_profile.get("study_date"),
    )
    metastatic_payload.setdefault(
        "testosterone",
        truth_values.get("testosterone") or latest_inputs.get("testosterone") or latest_inputs.get("testosterone_value"),
    )
    metastatic_payload.setdefault(
        "testosterone_date",
        truth_values.get("testosterone_date") or latest_followup.get("visit_date") or latest_inputs.get("testosterone_date"),
    )
    metastatic_payload.setdefault(
        "progression_pattern",
        truth_values.get("progression_pattern") or latest_inputs.get("progression_pattern") or latest_followup.get("progression_pattern"),
    )
    metastatic_payload.setdefault(
        "disease_status",
        latest_followup.get("disease_status") or latest_inputs.get("disease_status"),
    )

    context = resolve_metastatic_state_context(metastatic_payload)
    metastatic_payload["m_substage_resolved"] = context.get("m_substage_resolved_legacy")

    castrate_status = str(
        truth_values.get("castrate_testosterone_status")
        or latest_inputs.get("castrate_testosterone_status")
        or _derive_castrate_status(patient, state)
        or ""
    )
    conventional_imaging_status = str(metastatic_payload.get("conventional_imaging_status") or "").strip().upper()
    progression_required_fields = [
        "current_adt_context",
        "testosterone",
        "testosterone_date",
        "progression_pattern",
    ...
    progression_missing_fields = []
    for field in progression_required_fields:
        value = metastatic_payload.get(field)
        if field == "testosterone":
            if not _is_present(value) and not _is_present(castrate_status):
                progression_missing_fields.append(field)
        elif field == "conventional_imaging_status":
            if not _is_present(value):
                progression_missing_fields.append(field)
        elif not _is_present(value):
            progression_missing_fields.append(field)

    progression_verification_required = bool(context.get("metastatic_known")) and (
        state in {"adt_progression_verification", "m0_crpc", "m1_crpc"}
        or str(metastatic_payload.get("current_adt_context") or "").strip().lower() not in {"", "none"}
    )
    nmcrpc_eligible = (
        not context.get("metastatic_known")
        and conventional_imaging_status == "M0"
    )

    if nmcrpc_eligible:
        nmcrpc_ineligibility_reason = ""
    elif context.get("metastatic_known"):
        detection_basis = str(context.get("metastatic_detection_basis") or "unknown")
        if str(context.get("restaging_update_required") or "").lower() in {"1", "true", "yes"}:
            nmcrpc_ineligibility_reason = str(context.get("restaging_update_reason") or "")
        elif detection_basis == "psma_only":
            nmcrpc_ineligibility_reason = "Metástasis documentadas por PET PSMA; el caso ya corresponde a M1."
        elif detection_basis == "conventional":
            nmcrpc_ineligibility_reason = "Metástasis documentadas por imagen convencional; el caso ya corresponde a M1."
        elif detection_basis == "both":
            nmcrpc_ineligibility_reason = "Metástasis documentadas por PET PSMA e imagen convencional; el caso ya corresponde a M1."
        else:
            nmcrpc_ineligibility_reason = "Existe enfermedad metastásica documentada; m0_crpc ya no es elegible."
    else:
        nmcrpc_ineligibility_reason = "No debe intensificarse como nmCRPC hasta documentar que la enfermedad sigue siendo M0 en imagen convencional."

    target_if_confirmed = "m1_crpc" if context.get("metastatic_known") else ""
    target_if_not_castrate = _infer_metastatic_sensitive_target_state(patient, metastatic_payload) if context.get("metastatic_known") else ""

    state_reclassification_required = state == "m0_crpc" and bool(context.get("metastatic_known"))
    state_reclassification_reason = (
        "El último carril nmCRPC ya no es compatible con la evidencia anatómica actual y debe salir de m0_crpc."
        if state_reclassification_required
        else ""
    )

    return {
        **context,
        "m_substage_resolved": context.get("m_substage_resolved_legacy") or "M0",
        ...
        "state_reclassification_reason": state_reclassification_reason,
        "progression_verification_required": progression_verification_required,
        "progression_verification_required_fields": progression_required_fields if progression_verification_required else [],
        "progression_verification_missing_fields": progression_missing_fields if progression_verification_required else [],
        "progression_verification_target_state_if_confirmed": target_if_confirmed,
        "progression_verification_target_state_if_not_castrate": target_if_not_castrate,
    }
...
def build_state_classifier_payload(patient: dict[str, Any], latest_assessment: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    metastatic_state_bundle = _derive_metastatic_state_bundle(patient, latest_assessment, state)
    ...
    state_payload = {
        ...
        "metastatic_disease_known": 1 if metastatic_state_bundle.get("metastatic_known") else 0,
        "m_substage_resolved": metastatic_state_bundle.get("m_substage_resolved") or burden_context.get("m_substage_resolved") or "",
        "metastatic_detection_basis": metastatic_state_bundle.get("metastatic_detection_basis") or "",
        "metastasis_assessment_date": metastatic_state_bundle.get("metastasis_assessment_date") or "",
        "restaging_currentness_status": metastatic_state_bundle.get("restaging_currentness_status") or "",
        ...
        "conventional_imaging_date": current_values.get("conventional_imaging_date") or metastatic_state_bundle.get("metastasis_assessment_date") or "",
...
def build_clinical_signals(patient: dict[str, Any], latest_assessment: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if metastatic_state_bundle.get("metastatic_known"):
        metastatic_detail = metastatic_state_bundle.get("metastatic_stage_label") or metastatic_state_bundle.get("metastatic_stage_resolved")
        detection_basis = str(metastatic_state_bundle.get("metastatic_detection_basis") or "unknown")
        detail_suffix = {
            "psma_only": "documentado por PET PSMA",
            "conventional": "documentado por imagen convencional",
            "both": "documentado por PET PSMA e imagen convencional",
            "unknown": "documentado en el longitudinal",
        }.get(detection_basis, "documentado en el longitudinal")
        signals.append(
            {
                "key": "metastatic_state",
                "label": "Estado metastásico anatómico",
                "value": metastatic_detail,
                "status": "warning" if state == "adt_progression_verification" else "informative",
                "detail": f"{metastatic_detail} {detail_suffix}.".strip(),
            }
        )
        if metastatic_state_bundle.get("restaging_update_required"):
            awaiting_review.append(str(metastatic_state_bundle.get("restaging_update_reason") or "Se requiere restadificación actualizada."))
    ...
    if state in ADVANCED_STATES | MHSPC_STATES:
        ...
        hepatic_bundle = dict(truth_values.get("hepatic_safety_bundle") or latest_followup.get("hepatic_safety_bundle") or {})
        if hepatic_bundle.get("hepatic_risk_present") or str(truth_values.get("hepatic_risk_status") or latest_followup.get("hepatic_risk_status") or latest_followup.get("hepatic_risk_factors") or "").strip():
        ...
        if metastatic_state_bundle.get("progression_verification_required"):
            for field in list(metastatic_state_bundle.get("progression_verification_missing_fields") or [])[:]:
                label = {
                    "current_adt_context": "Contexto actual de ADT",
                    "testosterone": "Testosterona sérica actual",
                    "testosterone_date": "Fecha de testosterona actual",
                    "progression_pattern": "Patrón de progresión documentado",
                    "psa": "PSA actual",
                    "conventional_imaging_status": "Estado de imagen convencional",
                ...
                if label not in critical_missing:
    ...
    return ClinicalSignalSet(
        ...
        "supporting_evidence": reconciliation.get("supporting_evidence", {}),
        "metastatic_state_bundle": metastatic_state_bundle,
        "metastatic_stage_resolved": metastatic_state_bundle.get("metastatic_stage_resolved", "M0"),
        "metastatic_stage_label": metastatic_state_bundle.get("metastatic_stage_label", "M0"),
        "m_substage_resolved": metastatic_state_bundle.get("m_substage_resolved", "M0"),
        "metastatic_detection_basis": metastatic_state_bundle.get("metastatic_detection_basis", "unknown"),
        "nmcrpc_eligible": bool(metastatic_state_bundle.get("nmcrpc_eligible")),
        "nmcrpc_ineligibility_reason": metastatic_state_bundle.get("nmcrpc_ineligibility_reason", ""),
        "state_reclassification_required": bool(metastatic_state_bundle.get("state_reclassification_required")),
        "state_reclassification_reason": metastatic_state_bundle.get("state_reclassification_reason", ""),
        "restaging_update_required": bool(metastatic_state_bundle.get("restaging_update_required")),
        "restaging_currentness_status": metastatic_state_bundle.get("restaging_currentness_status", "unknown"),
        "restaging_update_reason": metastatic_state_bundle.get("restaging_update_reason", ""),
        "progression_verification_required": bool(metastatic_state_bundle.get("progression_verification_required")),
        "progression_verification_required_fields": list(metastatic_state_bundle.get("progression_verification_required_fields") or []),
        "progression_verification_missing_fields": list(metastatic_state_bundle.get("progression_verification_missing_fields") or []),
        "progression_verification_target_state_if_confirmed": metastatic_state_bundle.get("progression_verification_target_state_if_confirmed", ""),
        "progression_verification_target_state_if_not_castrate": metastatic_state_bundle.get("progression_verification_target_state_if_not_castrate", ""),
...
def build_next_best_action(
    ...
    dre_suspicious = str(
        truth_values.get("dre_suspicious")
        or truth_values.get("dre_finding")
        or latest_inputs.get("dre_suspicious")
        or latest_inputs.get("dre_finding")
        or ""
    ).strip().lower() in {"1", "true", "yes", "si", "sí", "suspicious", "sospechoso", "abnormal", "anormal"}
    repeat_psa_value = _safe_float(
        truth_values.get("repeat_psa_value")
        or latest_inputs.get("repeat_psa_value")
    )
    repeat_psa_date = _first_nonempty(
        truth_values.get("repeat_psa_date"),
        latest_inputs.get("repeat_psa_date"),
    )
    repeat_psa_required = bool(
        truth_values.get("repeat_psa_required")
        or (
            psa is not None
            and 3 <= psa <= 10
            and not dre_suspicious
        )
    )
    repeat_psa_done = repeat_psa_value is not None and _is_present(repeat_psa_date)
    negative_biopsy = (
        (state == "post_negative_biopsy_followup")
        and latest_biopsy
        and (_safe_int(latest_biopsy.get("positive_cores")) or 0) == 0
    )
    persistent_suspicion = negative_biopsy and (
        pirads_score >= 4
    ...
    life_expectancy_years = _safe_float(truth_values.get("life_expectancy_years") or latest_inputs.get("life_expectancy_years"))
    frailty_status = str(truth_values.get("frailty_status") or latest_inputs.get("frailty_status") or "").strip().lower()
    surgery_fitness = str(
        truth_values.get("anesthesia_surgical_fitness")
        or latest_inputs.get("anesthesia_surgical_fitness")
        or ""
    ).strip().lower()
    radiotherapy_feasibility = str(
        truth_values.get("radiotherapy_feasibility")
        or latest_inputs.get("radiotherapy_feasibility")
        or ""
    ).strip().lower()
    patient_priority = str(
        truth_values.get("patient_priority_profile")
        or latest_inputs.get("patient_priority_profile")
        or ""
    ).strip().lower()
    recommended_option_lower = recommended_option.lower()
    action_title = ""
    action_rationale = ""
    transition_title = ""
    ...
    def _localized_title() -> tuple[str, str]:
        if "observ" in recommended_option_lower or (
            frailty_status == "frail"
            and (life_expectancy_years is not None and life_expectancy_years <= 10)
        ):
            return (
                "Priorizar observación clínica",
                "La fragilidad y la expectativa de vida limitada hacen más razonable una conducta conservadora centrada en calidad de vida que un tratamiento definitivo agresivo.",
            )
        if "vigil" in recommended_option_lower or (
            isup_grade <= 1
            and not has_unfavorable_local_features
            ...
            return (
                "Sostener vigilancia activa",
                "El escenario actual sigue siendo compatible con vigilancia activa con monitorización protocolizada y reevaluación seriada.",
            )
        if "prostatect" in recommended_option_lower or (
            surgery_fitness == "fit"
            and patient_priority == "max_control_oncologico"
            and not ("radiot" in recommended_option_lower and ("adt" in recommended_option_lower or "abirater" in recommended_option_lower))
        ):
            return (
                "Priorizar prostatectomía radical",
                "El perfil funcional y la prioridad oncológica actual favorecen resolver con cirugía radical en lugar de mantener una ruta conservadora o genérica.",
            )
        if "abirater" in recommended_option_lower:
            return (
                "Priorizar radioterapia externa + terapia de privación androgénica + abiraterona",
                "La enfermedad localizada de muy alto riesgo ya favorece intensificación hormonal con RT definitiva cuando la vía quirúrgica no domina.",
            )
        if "radiot" in recommended_option_lower and ("privación" in recommended_option_lower or "privacion" in recommended_option_lower or "adt" in recommended_option_lower):
            return (
                "Priorizar radioterapia + terapia de privación androgénica",
                "El perfil localizado desfavorable favorece RT definitiva con intensificación hormonal en vez de una narrativa local inespecífica.",
            )
        if "radiot" in recommended_option_lower:
            return (
                "Priorizar radioterapia definitiva",
                "La enfermedad localizada actual favorece un control local con radioterapia frente a vigilancia simple.",
            )
    ...
    if proposals:
        ...
        transition_target_state = str(proposal.get("target_state") or "")
        transition_title = f"Confirmar transición a {_state_label(transition_target_state)}"
        transition_rationale = str(
            proposal.get("rationale")
            or "La nueva información longitudinal ya cambió la etapa clínica esperada."
        )
        recommendation_family = _proposal_recommendation_family(recommendation_family, proposal)

    if state in DIAGNOSTIC_STATES:
        if state == "post_negative_biopsy_followup" and persistent_suspicion:
            action_title = "Reabrir estudio diagnóstico con MRI, PSAD y decisión de rebiopsia"
            action_rationale = (
                "Persisten señales de sospecha clínicamente relevantes tras una biopsia benigna inicial y la conducta correcta es reabrir el estudio diagnóstico, no vigilancia pasiva."
            )
        elif repeat_psa_required and not repeat_psa_done:
            action_title = "Repetir PSA antes de cerrar la decisión diagnóstica"
            action_rationale = (
                "En un PSA inicial entre 3 y 10 ng/mL sin DRE claramente sospechoso, la repetición estructurada del PSA sigue siendo un paso decisional antes de escalar o cerrar la ruta diagnóstica."
            )
        elif (
            ...
            action_title = "Acelerar biopsia dirigida y confirmación histológica"
            action_rationale = "La señal radiológica o clínica de alto riesgo ya justifica cierre diagnóstico rápido con histología."
        else:
            action_title = "Mantener seguimiento diagnóstico con PSA y MRI seriados"
            action_rationale = "La etapa diagnóstica actual favorece vigilancia estrecha y repetición estructurada de PSA/MRI antes de escalar la invasividad."
    elif state == "localized_initial":
        action_title, action_rationale = _localized_title()
        ...
        if state == "post_radiotherapy_or_local_salvage":
            ...
            if transition_status == "redirect_systemic":
                ...
                action_title = f"Priorizar {dominant_label or 'terapia dirigida a metástasis (MDT) guiada por PSMA'}"
                ...
                action_title = f"Priorizar {dominant_label}" if dominant_label else "Sostener salvage local post-RT"
            ...
            action_title = "Iniciar evaluación temprana de salvage por PSA persistente"
            action_rationale = "El PSA persistente posoperatorio obliga a recalcular PSADT, factibilidad local e imagen si cambia la conducta, sin forzar todavía una BCR verdadera."
        elif state == "recurrence_bcr" or (state == "post_prostatectomy" and post_prostatectomy_course == "true_bcr"):
            if any(token in recommended_option_lower for token in ("enzalut", "apalut", "darolut", "abirater", "leupro")):
                action_title = f"Priorizar {recommended_option}"
                action_rationale = "La ruta postlocal ya perdió dominancia curativa aislada y la siguiente conducta pasa a una intensificación sistémica explícita."
            elif psma_pattern == "diseminado" or psma_stage_after in {"M1B", "M1C"}:
                action_title = "Redirigir a intensificación sistémica y staging avanzado"
                action_rationale = "La PSMA estructurada ya sugiere enfermedad diseminada y reduce la prioridad de rescate local aislado."
            else:
                action_title = "Activar salvage y reestadificación dirigida"
                action_rationale = "La recurrencia bioquímica posoperatoria exige definir ventana de rescate, PSADT e imagen dirigida sin demoras."
        else:
            action_title = "Mantener vigilancia post prostatectomía y control bioquímico"
            action_rationale = "El seguimiento posoperatorio estable exige PSA ultrasensible seriado, revisión funcional y preservación de la ventana de rescate."
    else:
        action_title = "Reevaluar secuencia sistémica y seguridad activa"
        action_rationale = "La enfermedad avanzada debe balancear elegibilidad terapéutica, biomarcadores y toxicidad."
        ...
        if state == "adt_progression_verification" and castrate_status == "not_castrate":
            action_title = "Optimizar ADT y confirmar testosterona en rango de castración"
            action_rationale = "La progresión no debe rotularse como CRPC mientras la testosterona siga por encima del umbral de castración."
        metastatic_stage_resolved = str(signals.get("metastatic_stage_resolved") or truth_values.get("metastatic_stage_resolved") or "").strip()
        progression_verification_missing = list(signals.get("progression_verification_missing_fields") or [])
        restaging_update_required = bool(signals.get("restaging_update_required"))
        restaging_update_reason = str(signals.get("restaging_update_reason") or "").strip()
        if state == "adt_progression_verification" and metastatic_stage_resolved and metastatic_stage_resolved != "M0":
            action_title = "Confirmar castración y progresión resistente con M1 ya documentado"
            action_rationale = (
                "El estado anatómico ya corresponde a M1; ahora la decisión depende de cerrar testosterona, patrón de progresión y restadificación suficiente para definir si el curso final es m1CRPC o metastásico sensible."
            )
            if progression_verification_missing:
                action_rationale = (
                    f"El estado anatómico ya corresponde a M1; todavía faltan datos para cerrar castración/progresión: {', '.join(progressions for progressions in progression_verification_missing[:4])}."
                )
            if restaging_update_required and restaging_update_reason:
                action_rationale = f"{action_rationale} {restaging_update_reason}".strip()
        elif state == "adt_progression_verification" and str(truth_values.get("castrate_testosterone_status") or "") == "confirmed_castrate":
            action_title = "Confirmar progresión bajo ADT y redefinir secuencia sistémica"
            action_rationale = "La testosterona ya está en rango de castración; la conducta debe salir del carril de optimización de ADT y centrarse en progresión confirmada."
        runtime_contracts = _resolve_runtime_regimen_contracts(patient, state, latest_assessment)
        module_result = dict(runtime_contracts.get("module_result") or {})
        ...
        if trigger_status == "confirm" and not safety_priority:
            action_title = "Completar datos críticos y recalcular la secuencia"
            recommendation_family = module_family
            action_rationale = str(sequence_bundle.get("line_change_reason") or "La conducta actual sigue condicionada por datos faltantes antes de cerrar la secuencia terapéutica.")
        elif preferred_label and trigger_status == "hold" and not safety_priority:
            action_title = f"Pausar y reevaluar {preferred_label}"
            recommendation_family = module_family
            action_rationale = str(sequence_bundle.get("line_change_reason") or monitoring_focus or "La toxicidad o seguridad activa obligan a pausar el regimen actual.")
        elif preferred_label and trigger_status in {"switch", "intensify"} and not safety_priority:
            action_title = f"Priorizar {preferred_label}"
            recommendation_family = module_family
            action_rationale = str(sequence_bundle.get("line_change_reason") or "La secuencia actual favorece un nuevo backbone preferente según elegibilidad y línea.")
            ...
            action_title = f"Redirigir la conducta {direction} con {preferred_label}"
            recommendation_family = module_family
            action_rationale = str(sequence_bundle.get("line_change_reason") or monitoring_focus or "La trayectoria clínica actual ya requiere redireccionar la siguiente línea.")
        elif preferred_label and trigger_status == "continue" and not safety_priority:
            action_title = f"Continuar {preferred_label}"
            recommendation_family = module_family
            action_rationale = str(sequence_bundle.get("line_change_reason") or monitoring_focus or "El backbone preferente actual sigue alineado con la trayectoria clínica.")
        if state == "m0_crpc" and preferred_label and not safety_priority:
            action_title = f"Priorizar {preferred_label}"
            recommendation_family = module_family
            action_rationale = "El nmCRPC longitudinal actual ya permite intensificación con el ARPI más consistente con riesgo y seguridad."
        elif (
            state == "m1_crpc"
            ...
            action_rationale = "La m1CRPC temprana sin biomarcadores de precisión dominantes ni exposición ARPI documentada favorece iniciar una vía ARPI antes de rutas posteriores."
        elif state == "m1_crpc" and preferred_label and not safety_priority:
            action_title = f"Priorizar {preferred_label}"
            recommendation_family = module_family
            action_rationale = "La secuencia mCRPC debe seguir la elegibilidad terapéutica longitudinal, biomarcadores accionables y seguridad vigente."
        elif state in MHSPC_STATES and preferred_label and not safety_priority:
            action_title = f"Priorizar {preferred_label}"
            recommendation_family = module_family
            action_rationale = "El mHSPC actual ya tiene suficiente contexto longitudinal para elegir el backbone sistémico más competitivo."
    ...
    response = NextBestAction(
        ...
        transition_target_state=transition_target_state,
        ...
        evidence_basis=(proposals[0].get("evidence_basis") if proposals else None) or COMMON_EVIDENCE,
    ).to_dict()
...
def build_longitudinal_intelligence_bundle(
    ...
    effective_state = followup_runtime.get("state") or signals.get("reconciled_state") or signals.get("state") or _current_state(patient, latest_assessment)
    effective_management_track = followup_runtime.get("management_track") or signals.get("reconciled_management_track") or signals.get("management_track") or infer_management_track(patient, signals.get("reconciled_state") or signals.get("state") or "", latest_assessment or patient.get("latest_assessment"))
    ...
    preferred_regimen = dict((latest_assessment or {}).get("result_snapshot", {}).get("preferred_frontline_regimen") or {})
    if not preferred_regimen:
        preferred_regimen = dict(runtime_regimen_contracts.get("preferred_regimen") or {})
    ...
    initial_care_intent_contract = build_care_intent_contract(
        patient=patient,
    ...
    next_best_action = _align_next_best_action_with_care_intent(
        next_best_action,
        initial_care_intent_contract,
        decision_input_requirements,
    )
    advanced_gate_signals = {
        **signals,
        "next_best_action": next_best_action,
    }
    ...
    advanced_release_gate = build_advanced_release_gate(
        ...
        signals=advanced_gate_signals,
        candidate_family=preferred_regimen.get("family_code") or "",
    )
    decision_input_requirements = merge_advanced_release_gate_into_requirements(
        decision_input_requirements,
        advanced_release_gate,
    )
    plan_signals["advanced_followup_bundle"] = advanced_followup_bundle
    plan_signals["staging_adjudication_bundle"] = staging_adjudication_bundle
    plan_signals["supportive_care_toxicity_readiness_bundle"] = supportive_care_toxicity_readiness_bundle
    plan_signals["advanced_release_gate"] = advanced_release_gate
    ...
    therapeutic_readiness_bundle = build_therapeutic_readiness_bundle(
        state=effective_state,
        phenotype_state=signals.get("phenotype_state") or signals.get("reconciled_state") or signals.get("state") or "",
        preferred_regimen=preferred_regimen,
        next_best_action=next_best_action,
        decision_input_requirements=decision_input_requirements,
        comparative_eligibility_matrix=runtime_regimen_contracts.get("comparative_eligibility_matrix") or {},
        systemic_regimen_scope_contract=runtime_regimen_contracts.get("systemic_regimen_scope_contract") or {},
        care_intent_contract=care_intent_contract,
        palliative_transition_bundle=palliative_transition_bundle,
        survivorship_transition_bundle=survivorship_transition_bundle,
        therapeutic_window_bundle=governance_bundle.get("therapeutic_window_bundle") or {},
        active_regimen_monitoring_package=runtime_regimen_contracts.get("active_regimen_monitoring_package") or {},
        recommendation_block_status=governance_bundle.get("recommendation_block_status") or "",
        recommendation_block_reason=governance_bundle.get("recommendation_block_reason") or "",
        allowed_actions_while_blocked=governance_bundle.get("allowed_actions_while_blocked") or [],
        signals={**signals, "next_best_action": next_best_action},
        advanced_followup_bundle=advanced_followup_bundle,
        staging_adjudication_bundle=staging_adjudication_bundle,
        supportive_care_toxicity_readiness_bundle=supportive_care_toxicity_readiness_bundle,
    ...
    plan_signals["therapeutic_readiness_bundle"] = therapeutic_readiness_bundle
    plan_signals["clinical_fact_bundle"] = clinical_fact_bundle
    plan_signals["decision_evidence_currentness_bundle"] = governance_bundle.get("decision_evidence_currentness_bundle") or {}
    ...
    return {
        "signals": signals,
        "metastatic_state_bundle": dict(signals.get("metastatic_state_bundle") or {}),
        ...
        "systemic_regimen_scope": runtime_regimen_contracts.get("systemic_regimen_scope", "not_applicable"),
        "systemic_regimen_scope_contract": runtime_regimen_contracts.get("systemic_regimen_scope_contract", {}),
        "palliative_transition_bundle": palliative_transition_bundle,
        "palliative_monitoring_package": palliative_monitoring_package,
