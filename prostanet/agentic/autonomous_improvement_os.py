...
def build_mission_control(
    *,
    ...
    top = (development_autodrive.get("today_queue") or [{}])[0]
...
def build_cortana_loop_interface(
    *,
    ...
    command_routes = [
        {
            **dict(command),
            "status": "available" if ready_9a else "blocked_until_9a_ready",
            "write_allowed": False,
            "merge_allowed": False,
...
def _score_candidate(candidate: Mapping[str, Any]) -> tuple[float, dict[str, Any]]:
    ...
    final_score = round((raw_score / effort) * blocker_penalty, 2)
    return final_score, {
...
def _candidate_actionable(candidate: Mapping[str, Any]) -> bool:
    ...
    return status in {"proposal_ready", "human_review_required", "validated"} and not candidate.get("blockers")
    ...
    return [
        {
            ...
            "field_aliases": ["psa", "psa_value", "baseline_psa", "pirads", "mri_pirads", "dre", "biopsy_status", "prior_negative_biopsy"],
            "required_fields": ["psa", "mri_pirads_or_reason", "dre", "biopsy_status"],
            "capture_surface": "clinical-hub classifier + diagnostic wizard + longitudinal-capture",
            ...
            "tests": ["test_diagnostic_truth_minimum_contract_routes_to_capture"],
        ...
        },
        {
            ...
            "required_fields": ["psa_series_or_psadt", "local_therapy_context", "pathology_margin_or_rt_nadir", "imaging_if_changes_management"],
            "capture_surface": "BCR wizard + longitudinal-capture?decision_lane=bcr_salvage_readiness",
            ...
            "tests": ["test_bcr_contract_blocks_crpc_parp_rlt_and_routes_psadt"],
        ...
        },
        {
            ...
            "field_aliases": ["patient_preference", "values", "goal_of_care", "pros", "epic26", "esas", "toxicity_tolerance", "tradeoff"],
...
def derive_contractual_gap_rows(patient_summary: Mapping[str, Any] | None) -> list[dict[str, Any]]:
...
def _decision_today_candidates(context: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    missing_labels = [str(pair[0]) for pair in missing[:5]]
    return [_candidate(
        lane="data_integrity_gap" if missing_labels else "critical_clinical_gap",
        title="Close dominant fields blocking canonical Decision Today",
        description=(
            f"{blocked}/{evaluated} scanned patients have Decision Today blocked or requiring data. "
            f"Dominant missing fields: {', '.join(missing_labels) or 'not classified'}."
        ),
        clinical_impact=9,
        severity=8,
        effort_h=3.0,
        ...
        patient_scope={
            ...
            "missing_fields_top": missing[:8],
        },
        contract_gaps=derive_contractual_gap_rows(patients)[:8],
...
def _patient_twin_readiness_value(ctx: Mapping[str, Any]) -> float:
    ...
    return round(max(0.0, min(45.0, value)), 2)
...
def _mission_metric_reason(metric: Mapping[str, Any], gap_bundle: Mapping[str, Any]) -> str:
    ...
    if key == "patient_twin_readiness":
        return "Patient Twin aún no está plenamente implementado o disponible como read-model clínico."
