...
def _score_cve_clean() -> tuple[float, list[Gap]]:
    """Score 1.0 si pip-audit limpio. Si pip-audit no instalado → 0.5 (unknown)."""
    try:
        proc = subprocess.run(
        ...
        # tool not installed — neutral score with informational gap
        return 0.5, [Gap(
            pillar_id=3,
            kind="cve_scan_tool_missing",
            description="pip-audit not installed; CVE compliance unknown (install: pip install pip-audit)",
